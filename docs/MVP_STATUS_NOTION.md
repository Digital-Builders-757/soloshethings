...
- **Supabase migration CI guardrails (2026-05)** - `develop`: path-filtered workflow validates linked staging (`db push --dry-run` + `db lint` only, no deploy). `main`: path-filtered `db push` gated by **`environment: production`** (team configures required reviewers in GitHub). Doc: **`docs/supabase-readonly-migration-verification.md`** for read-only prod checks before removing repair migrations; **`docs/procedures/MIGRATION_PROCEDURE.md`** updated.
